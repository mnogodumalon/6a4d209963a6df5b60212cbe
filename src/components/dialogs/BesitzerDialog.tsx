...
interface BesitzerDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Besitzer['fields']) => Promise<void>;
  defaultValues?: Besitzer['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BesitzerDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: BesitzerDialogProps) {
  const [fields, setFields] = useState<Partial<Besitzer['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'besitzer');
      await onSubmit(clean as Besitzer['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Besitzer['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Besitzer bearbeiten' : 'Besitzer hinzufügen';
