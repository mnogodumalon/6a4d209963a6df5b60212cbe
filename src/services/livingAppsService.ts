...
import type { Besitzer, Schallplatten, CreateBesitzer, CreateSchallplatten } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getBesitzer(): Promise<Besitzer[]> {
    ...
    return enrichLookupFields(records, 'besitzer');
  }
  static async getBesitzerEntry(id: string): Promise<Besitzer | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Besitzer;
    return enrichLookupFields([record], 'besitzer')[0];
  }
  static async createBesitzerEntry(fields: CreateBesitzer) {
    return callApi('POST', `/apps/${APP_IDS.BESITZER}/records`, { fields: cleanFieldsForApi(fields as any, 'besitzer') });
  }
  static async updateBesitzerEntry(id: string, fields: Partial<CreateBesitzer>) {
    return callApi('PATCH', `/apps/${APP_IDS.BESITZER}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'besitzer') });
  }
  ...
  // --- SCHALLPLATTEN ---
  static async getSchallplatten(): Promise<Schallplatten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHALLPLATTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Schallplatten[];
    return enrichLookupFields(records, 'schallplatten');
  }
  static async getSchallplattenEntry(id: string): Promise<Schallplatten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHALLPLATTEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Schallplatten;
    return enrichLookupFields([record], 'schallplatten')[0];
  }
  static async createSchallplattenEntry(fields: CreateSchallplatten) {
    return callApi('POST', `/apps/${APP_IDS.SCHALLPLATTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schallplatten') });
  }
  static async updateSchallplattenEntry(id: string, fields: Partial<CreateSchallplatten>) {
    return callApi('PATCH', `/apps/${APP_IDS.SCHALLPLATTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schallplatten') });
  }
  static async deleteSchallplattenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHALLPLATTEN}/records/${id}`);
  }
