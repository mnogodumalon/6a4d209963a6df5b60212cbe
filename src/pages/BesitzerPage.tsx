...
export default function BesitzerPage() {
  const [records, setRecords] = useState<Besitzer[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Besitzer | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Besitzer | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Besitzer | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getBesitzer());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Besitzer['fields']) {
    await LivingAppsService.createBesitzerEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Besitzer['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBesitzerEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBesitzerEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Besitzer"
      subtitle={`${records.length} Besitzer im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Besitzer. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
