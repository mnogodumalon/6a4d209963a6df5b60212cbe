...
const SCHALLPLATTEN_FIELDS = [
  { key: 'titel', label: 'Titel', type: 'string/text' },
  { key: 'kuenstler', label: 'Künstler / Band', type: 'string/text' },
  { key: 'genre', label: 'Genre', type: 'lookup/select', options: [{ key: 'rock', label: 'Rock' }, { key: 'pop', label: 'Pop' }, { key: 'jazz', label: 'Jazz' }, { key: 'klassik', label: 'Klassik' }, { key: 'blues', label: 'Blues' }, { key: 'soul', label: 'Soul' }, { key: 'electronic', label: 'Electronic' }, { key: 'hip_hop', label: 'Hip-Hop' }, { key: 'country', label: 'Country' }, { key: 'sonstige', label: 'Sonstige' }] },
  { key: 'erscheinungsjahr', label: 'Erscheinungsjahr', type: 'number' },
  { key: 'label', label: 'Label', type: 'string/text' },
  { key: 'zustand', label: 'Zustand', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'sehr_gut', label: 'Sehr gut' }, { key: 'gut', label: 'Gut' }, { key: 'befriedigend', label: 'Befriedigend' }, { key: 'akzeptabel', label: 'Akzeptabel' }, { key: 'schlecht', label: 'Schlecht' }] },
  { key: 'besitzer_ref', label: 'Besitzer', type: 'applookup/select', targetEntity: 'besitzer', targetAppId: 'BESITZER', displayField: 'vorname' },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'besitzer', label: 'Besitzer', pascal: 'Besitzer' },
  { key: 'schallplatten', label: 'Schallplatten', pascal: 'Schallplatten' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schallplatten': return (data as any).schallplatten as Schallplatten[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schallplatten' && fieldKey === 'besitzer_ref') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'besitzer': return BESITZER_FIELDS;
      case 'schallplatten': return SCHALLPLATTEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schallplatten': return {
        create: (fields: any) => LivingAppsService.createSchallplattenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchallplattenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchallplattenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'schallplatten' || dialogState?.entity === 'schallplatten') && (
        <SchallplattenDialog
          open={createEntity === 'schallplatten' || dialogState?.entity === 'schallplatten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schallplatten' ? handleUpdate : (fields: any) => handleCreate('schallplatten', fields)}
          defaultValues={dialogState?.entity === 'schallplatten' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'schallplatten' && (
        <SchallplattenViewDialog
          open={viewState?.entity === 'schallplatten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schallplatten', record: r }); }}
          besitzerList={(data as any).besitzer ?? []}
        />
      )}
