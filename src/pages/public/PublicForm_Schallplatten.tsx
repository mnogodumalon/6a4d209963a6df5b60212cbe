...
const PROXY_BASE = '';
const APP_ID = '6a4d208b79777fe4a25678b0';
...
export default function PublicFormSchallplatten() {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-foreground">Schallplatten — Formular</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 bg-card rounded-xl border border-border p-6 shadow-md">
          ...
          <div className="space-y-2">
            <Label htmlFor="kuenstler">Künstler / Band</Label>
            <Input
              id="kuenstler"
              value={fields.kuenstler ?? ''}
              onChange={e => setFields(f => ({ ...f, kuenstler: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="genre">Genre</Label>
            <Select
              value={lookupKey(fields.genre) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, genre: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="genre"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="rock">Rock</SelectItem>
                <SelectItem value="pop">Pop</SelectItem>
                <SelectItem value="jazz">Jazz</SelectItem>
                <SelectItem value="klassik">Klassik</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="erscheinungsjahr">Erscheinungsjahr</Label>
            <Input
              id="erscheinungsjahr"
              type="number"
              value={fields.erscheinungsjahr ?? ''}
              onChange={e => setFields(f => ({ ...f, erscheinungsjahr: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
